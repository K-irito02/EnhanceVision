...
5. **测试循环验证**：启动程序后等待用户手动测试，通过弹出的询问卡片选项决定是否进行日志检查，循环直到问题解决
...
2. **用户确认**：用户测试完成后，通过弹出询问卡片选项决定是否检查日志
3. **日志检查**：如果用户选择检查日志，自动读取并分析日志文件中的错误和警告
4. **问题诊断**：输出发现的问题信息
5. **循环验证**：如果发现问题，返回修复流程；如果无问题，询问用户是否还有问题需要修复
...
- **测试循环验证**：启动程序后等待用户手动测试，通过弹出询问卡片选项决定是否进行日志检查，循环直到问题解决
