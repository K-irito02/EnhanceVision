...
| 构建后验证 | 每次成功构建后运行程序验证 |
| 错误循环修复 | 持续修复直到零错误零警告 |
| 测试循环验证 | 启动程序后等待用户测试，弹出询问卡片，通过选项决定是否查看运行后的测试日志，也可以用户选项引导用户输入其他文字消息告知测试发现的其它问题 |
...
- **测试循环验证**：启动程序后等待用户手动测试，通过弹出询问卡片，通过选项决定是否查看运行后的测试日志，也可以用户选项引导用户输入其他文字消息告知测试发现的其它问题
