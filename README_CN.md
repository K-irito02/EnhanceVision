...
- Windows 安装版现已在安装器完成页启动和常规快捷方式启动两种场景下都保持拖拽导入可用
- 若应用被误以管理员权限启动，启动阶段会自动尝试降权重启，以维持拖拽导入可用性
...
- 安装完成后的验收建议包含一次资源管理器拖拽导入验证，确认图片/视频可直接拖入应用
- 发布验收建议增加一次“手动管理员启动”检查：应自动降权重启，若失败需弹出明确提示
