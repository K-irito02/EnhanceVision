...
description: "Qt 项目构建和调试专家 - 自动化 EnhanceVision 项目（Qt Quick + C++ + NCNN）的构建、运行、调试和修复。当用户要求构建项目、运行程序、解决编译错误、调试运行时问题、检查代码质量、配置环境或任何与构建相关的问题时，必须使用此技能。"
---
...
| 构建后验证 | 每次成功构建后运行程序验证 |
| 错误循环修复 | 持续修复直到零错误零警告 |
| 测试循环验证 | 启动程序后等待用户测试，弹出询问卡片，通过 y/n 确认是否进行测试，用户也可以输入其他文字消息告知测试发现的问题，用户确认后，程序会自动进行日志检查 |
| 本地调试优先 | 所有调试操作在本地编辑器完成，不依赖第三方工具 |
...
| 模块 | 文件 | 用途 |
|------|------|------|
| 执行流程 | `build/workflow.md` | 构建执行步骤、测试循环 |
| 构建优化 | `build/optimization.md` | PCH、多进程编译、缓存 |
...
- **测试循环验证**：启动程序后等待用户手动测试，通过弹出询问卡片，用户输入 y/n 确认是否进行日志检查，用户也可以输入其他文字消息告知测试发现的问题
- **本地调试优先**：所有调试操作通过添加日志、分析日志、查看系统信息等方式在本地完成
- **第三方工具禁用**：不在外部编辑器或调试器中执行调试操作
